...
但是，在实际实现中，需要注意的一个问题是，我们上面一次性全部生成 $L+1$ 个候选 latent 变量，可能会导致显存占用过高。因此，在实际代码中，我们会将采样过程分块（chunk）进行，以控制显存使用峰值。此时复杂度与采样步数 $L$ 呈正相关。极限情况，当 chunk size 设为 1 时，复杂度退化回 $O\big((L+1)T_q\big)$。
